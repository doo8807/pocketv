...
JNIEXPORT jint JNICALL Java_com_example_doo88_pocketv_NDK_run_1ffmpeg(JNIEnv *env, jobject obj, jstring roomnumber)

{
    ...
    // ffmpeg -i /sdcard/input.mp4 -filter:v crop=300:400:10:20 /sdcard/output.mp4
        ...
        const char *room = (*env)->GetStringUTFChars(env, roomnumber, NULL);


                 (*env)->ReleaseStringUTFChars(env, roomnumber, room);


    char *s1 = malloc(sizeof(char) * 50);    // char 20개 크기만큼 동적 메모리 할당
    strcpy(s1, "/sdcard/");   // s2에 Hello 문자열 복사
    strcat(s1, room);
    strcat(s1, ".mp4");

    char *s2 = malloc(sizeof(char) * 50);    // char 20개 크기만큼 동적 메모리 할당
    strcpy(s2, "rtmp://119.207.144.112:8805/src/");   // s2에 Hello 문자열 복사
    strcat(s2, room);
